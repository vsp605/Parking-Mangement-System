...
def admin_login():
    if request.method == 'POST':
        ...
        session['is_admin'] = True  # Distinct flag for admin users
...
def admin_add_features():
    ...
    if request.method == 'POST':
        ...
        try:
            cur = mysql.connection.cursor()
            cur.execute("INSERT INTO features (content) VALUES (%s)", (content,))
            mysql.connection.commit()
            cur.close()
...
def admin_add_guidelines():
    ...
    if request.method == 'POST':
        ...
        try:
            cur = mysql.connection.cursor()
            cur.execute("INSERT INTO guidelines (content) VALUES (%s)", (content,))
            mysql.connection.commit()
            cur.close()
...
def login():
    if request.method == 'POST':
        ...
        session['is_admin'] = False  # Explicitly mark as not an admin
...
def features():
    try:
        cur = mysql.connection.cursor()
        cur.execute("SELECT content FROM features")
...
def guidelines():
    try:
        cur = mysql.connection.cursor()
        cur.execute("SELECT content FROM guidelines")
