...
def book_slot():
    ...
    try:
        ...
        cur.execute("""
            UPDATE ParkingSlot 
            SET status = 'booked', user_id = %s 
            WHERE id = %s AND location = %s AND status = 'available'
        """, (user_id, slot_id, location))
        mysql.connection.commit()
...
def generate_bill():
    try:
        ...
        rendered = render_template('bill.html', bill_id=bill_id, payment_id=payment_id, slot_id=slot_id, amount=amount, date=date)

        pdf_output_path = os.path.join(app.config['UPLOAD_FOLDER'], f'bill_{bill_id}.pdf')
        pdfkit.from_string(rendered, pdf_output_path)
